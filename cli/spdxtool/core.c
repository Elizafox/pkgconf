...
spdxtool_core_creation_info_new(pkgconf_client_t *client, const char *agent_id, const char *id, const char *time)
{
	...
	if (!(creation->id = strdup(id)))
		goto oom;
	if (!(creation->created = time ? strdup(time) : spdxtool_util_get_current_iso8601_time()))
		goto oom;
	if (!(creation->created_by = strdup(agent_id)))
		goto oom;
	creation->created_using = "pkgconf spdxtool";
	if (!(creation->spec_version = strdup(spdxtool_util_get_spdx_version(client))))
		goto oom;
...
spdxtool_core_creation_info_to_object(pkgconf_client_t *client, const spdxtool_core_creation_info_t *creation)
{
	bool ok = false;
	spdxtool_serialize_value_t *ret = NULL;
	spdxtool_serialize_array_t *created_by = NULL;
	spdxtool_serialize_object_list_t *object_list = spdxtool_serialize_object_list_new();
	if (!object_list)
		goto oom;

	created_by = spdxtool_serialize_array_new();
	if (!created_by)
		goto oom;
	...
	ok = spdxtool_serialize_object_add_array(object_list, "createdBy", created_by);
	created_by = NULL;
	if (!ok)
		goto oom;

	if (!spdxtool_serialize_object_add_string(object_list, "specVersion", creation->spec_version))
		goto oom;
...
spdxtool_core_spdx_document_new(pkgconf_client_t *client, const char *spdx_id, const char *creation_id, const char *agent_id)
{
	...
	if (!(spdx->spdx_id = strdup(spdx_id)))
		goto oom;
	if (!(spdx->agent = strdup(agent_id)))
		goto oom;
	if (!(spdx->creation_info = strdup(creation_id)))
		goto oom;
...
spdxtool_core_spdx_document_to_object(pkgconf_client_t *client, spdxtool_core_spdx_document_t *spdx)
{
	...
	spdxtool_serialize_array_t *root_element_array = NULL, *element_array = NULL;
	spdxtool_serialize_object_list_t *object_list = spdxtool_serialize_object_list_new();
	if (!object_list)
		goto err;
	...
	PKGCONF_FOREACH_LIST_ENTRY(spdx->rootElement.head, iter)
	{
		...
		if (!pkg_spdx_id)
			goto err;
		ok = spdxtool_serialize_array_add_string(element_array, sbom->spdx_id) &&
...
err:
	if (!ret)
		pkgconf_error(client, "spdxtool_core_spdx_document_to_object: %s", errstr);
 ...
 *    :param const pkgconf_list_t *to: list of to spdxIds
 *    :param const char *relationship_type: These can be found on SPDX documentation
 *    :return: NULL if some problem occurs and SpdxDocument struct if not
 */
spdxtool_core_relationship_t *
spdxtool_core_relationship_new(pkgconf_client_t *client, const char *creation_info_id, const char *spdx_id, const char *from, const pkgconf_list_t *to, const char *relationship_type)
{
	...
	if (!(relationship->creation_info = strdup(creation_info_id)))
		goto oom;
	if (!(relationship->spdx_id = strdup(spdx_id)))
		goto oom;
	if (!(relationship->from = strdup(from)))
		goto oom;
	if (!(relationship->relationship_type = strdup(relationship_type)))
		goto oom;

 	if (!(relationship->to = calloc(1, sizeof(pkgconf_list_t))))
		goto oom;
	if (!pkgconf_license_copy_list(client, relationship->to, to))
		goto oom;
...
{
	spdxtool_serialize_value_t *ret = NULL;
	spdxtool_serialize_array_t *to = NULL;
	spdxtool_serialize_object_list_t *object_list = spdxtool_serialize_object_list_new();
	if (!object_list)
		goto oom;

	to = spdxtool_serialize_array_new();
	if (!to)
		goto oom;
	...
	if (!spdxtool_serialize_object_add_string(object_list, "type", relationship->type))
		goto oom;
	if (!spdxtool_serialize_object_add_string(object_list, "creationInfo", relationship->creation_info))
		goto oom;
	if (!spdxtool_serialize_object_add_string(object_list, "spdxId", relationship->spdx_id))
		goto oom;
	if (!spdxtool_serialize_object_add_string(object_list, "from", relationship->from))
		goto oom;
