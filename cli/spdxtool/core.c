...
spdxtool_core_spdx_document_to_object(pkgconf_client_t *client, spdxtool_core_spdx_document_t *spdx)
{
	...
	ok = spdxtool_serialize_object_add_string(object_list, "type", spdx->type) &&
...
spdxtool_core_relationship_to_object(pkgconf_client_t *client, const spdxtool_core_relationship_t *relationship)
{
	...
	if (!spdxtool_serialize_array_add_string(to, relationship->to))
		goto oom;
	...
	if (!spdxtool_serialize_object_add_string(object_list, "from", relationship->from))
		goto oom;
	ok = spdxtool_serialize_object_add_array(object_list, "to", to);
	to = NULL;
	if (!ok)
		goto oom;
