...
spdxtool_software_package_to_object(pkgconf_client_t *client, pkgconf_pkg_t *pkg, spdxtool_core_spdx_document_t *spdx)
{
	...
	if (!(spdxtool_serialize_object_add_string(object_list, "type", "software_Package") &&
		...
		spdxtool_serialize_object_add_string(object_list, "software_copyrightText", pkg->copyright ? pkg->copyright : "NOASSERTION") &&
		spdxtool_serialize_object_add_string(object_list, "software_homePage", pkg->url ? pkg->url : "") &&
		spdxtool_serialize_object_add_string(object_list, "software_downloadLocation", pkg->source ? pkg->source : "") &&
	...
	PKGCONF_FOREACH_LIST_ENTRY(pkg->license.head, node)
	{
		...
		{
			char *spdx_id_license = spdxtool_util_get_spdx_id_string(client, "simplelicensing_LicenseExpression", license->data);
			if (!spdx_id_license)
			{
	...
	if (tuple_license)
	{
		spdxtool_core_relationship_t *relationship = spdxtool_core_relationship_new(client, creation_info, tuple_license, spdx_id, &relations, "hasDeclaredLicense");
		free(tuple_license);
		if (!relationship)
			goto err;
	...
	if (tuple_license)
	{
		spdxtool_core_relationship_t *relationship = spdxtool_core_relationship_new(client, creation_info, tuple_license, spdx_id, &relations, "hasConcludedLicense");
		free(tuple_license);
		if (!relationship)
			goto err;
		spdxtool_core_spdx_document_add_relationship(client, spdx, relationship);
	...
	PKGCONF_FOREACH_LIST_ENTRY(pkg->required.head, node)
	{
		...
		pkgconf_license_insert(client, &relations, PKGCONF_LICENSE_UNKNOWN, spdx_id_package);
		spdxtool_core_relationship_t *relationship = spdxtool_core_relationship_new(client, creation_info, spdx_id_relation, spdx_id, &relations, "dependsOn");
		free(spdx_id_relation);
